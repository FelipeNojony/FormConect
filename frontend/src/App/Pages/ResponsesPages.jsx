...
export default function ResponsesPage() {
  ...
  const publishedForms = useMemo(() => {
    return JSON.parse(localStorage.getItem(PUBLISHED_FORMS_KEY) || "[]");
  }, []);
  ...
  const responses = useMemo(() => {
    ...
    return allResponses.filter((response) => response.formSlug === slug);
  }, [slug]);
  ...
  function getQuestionTitle(questionId) {
    if (!form?.questions) return questionId;
